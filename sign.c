                ...
                const unsigned char *pk)
{
  ...
  uint16_t nonce = 0;
  poly     c, chat;
  polyvecl mat[K], s1, y, yhat, z, z1;
  polyveck s2, t0, w, w1, t1;
  polyveck h, tmp, tmp1, tmp2;
  ...
  // printf("========Printing z after ntt in verify=======\n");
  //
  // for(i=0;i<K;i++)
  // {
  //     for(j=0;j<N;j++)
  //       printf("%d, ",t1.vec[i].coeffs[j]);
  //     printf("\n");
  // }
  ...
  for(i = 0; i < K ; ++i)
    polyvecl_pointwise_acc_invmontgomery(tmp1.vec+i, mat+i, &z1);
  ...
  polyveck_decompose(&tmp, &tmp2, &tmp1);
  n = polyveck_back_calculate_hint(&h, &tmp, &w1);
                     ...
                     const unsigned char *pk)
{
  ...
  *mlen = smlen - CRYPTO_BYTES;
  ...
  /* Reconstruct w1 */
  polyveck_freeze(&tmp1);
  polyveck_decompose(&tmp3,&tmp2,&tmp1);
