  ...
  for(i = 0; i < K; ++i)
    for(j = 0; j < N; ++j)
      v1->vec[i].coeffs[j] = decompose(v->vec[i].coeffs[j],
                                       &v0->vec[i].coeffs[j]);
      // printf("v1 = %d, v2 = %d, v3 = %d\n",v1->vec[i].coeffs[j],v->vec[i].coeffs[j],v0->vec[i].coeffs[j]);
}
...
/*************************************************
* Name:        polyveck_back_calculate_hint
*
* Description: Compute hint vector. The coefficients of the polynomials indicate
*              whether or not the high bits of the corresponding coefficients
*              of the input polynomials differ.
...
* Returns number of 1 bits.
**************************************************/
unsigned int polyveck_back_calculate_hint(polyveck *h,
                                const polyveck *u,
                                const polyveck *v)
{
  unsigned int i, j, s = 0;
  ...
  for(i = 0; i < K; ++i)
    for(j = 0; j < N; ++j) {
        if(u->vec[i].coeffs[j] != v->vec[i].coeffs[j])
            h->vec[i].coeffs[j] = 1;
        else
            h->vec[i].coeffs[j] = 0;
      s += h->vec[i].coeffs[j];
